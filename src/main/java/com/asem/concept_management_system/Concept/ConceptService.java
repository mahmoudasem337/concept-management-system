...
public class ConceptService {
    ...
    public List<ConceptResponse> getAllConcepts() {
        List<ConceptResponse> concepts = conceptRepository.findAllBy();
        return concepts.stream()
                .map(concept -> new ConceptResponse(concept.getTitle(), concept.getDescription(), concept.getDifficultyLevel(), concept.getStatus()))
                .toList();
    }
    ...
    public ConceptResponse updateConcept(String id, Concept updatedData) {
        ...
        ConceptResponse response = new ConceptResponse(existing.getTitle(), existing.getDescription(), existing.getDifficultyLevel(),existing.getStatus());
    ...
    Concept concept = conceptRepository.findByTitle(title)
                .orElseThrow(() -> new ResourceNotFoundException("Concept not found"));
